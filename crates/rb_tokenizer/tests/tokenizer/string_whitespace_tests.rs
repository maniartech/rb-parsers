...
fn get_string_tokenizer() -> Tokenizer {
    ...
    tokenizer.add_regex_scanner(r#"^"([^"\\]|\\.)*""#, "String", Some("DoubleQuoted")).unwrap();
    tokenizer.add_regex_scanner(r#"^'([^'\\]|\\.)*'"#, "String", Some("SingleQuoted")).unwrap();
    // Backtick strings are raw strings - no escape scanning
    tokenizer.add_regex_scanner(r#"^`[^`]*`"#, "String", Some("Backtick")).unwrap();
